...
import { ArrowLeft, BarChart3, ChevronDown, ChevronRight, Loader2, Pencil, Plus, RefreshCw, Users, X } from 'lucide-react';
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (
                ...
                todoItems.length === 0 &&
                (featureData.feature_list || []).length > 0 &&
                (
                    normalizeId(user?.id) === normalizeId(proj?.pm_user_id)
                    || normalizeId(user?.id) === normalizeId(proj?.owner_user_id)
                )
            ) {
                autoGeneratedRef.current = true;
                await syncFeatureTasks();
            }
        } catch (err) {
